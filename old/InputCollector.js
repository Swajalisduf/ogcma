function InputCollector() {
  ...
  this.collectCreationDateInput = () => {
    ...
    //This commented out code is to prefix the date with
    // "ca. or "before" and postfix it with "bc" or "ad".
    // Implement once creationDate.php is finished.
    // if (date != "") {
  ...
  this.collectAuthorInfoInput = () => {
    let authorInputs = [];
    authorInputs.push(["Author's Given Names", document.getElementsByName('author')[0].value]);
    authorInputs.push(["Author's Surname", document.getElementsByName('author')[1].value]);
    authorInputs.push(["Author's Birth and Death Dates", document.getElementsByName('author')[2].value]);
    authorInputs.push(["Additional Author Information", document.getElementsByName('author')[3].value]);
    return authorInputs;
  }
